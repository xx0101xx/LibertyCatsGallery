  ...
  <div id="gamePage">
    ...
    <a-row align="center">
      <div v-show="gameStatus > 0" class="level-board">
        <div v-for="(block, idx) in levelBlocksVal" :key="idx">
          ...
          >
            <img class="cat-img" :src="`src/assets/cat/${block.type}`" alt="" />
            <!-- {{ block.type }} -->
          </div>
        </div>
      </div>
    ...
    <a-row align="space-between" class="random-board">
      ...
      >
        ...
        >
          <!-- {{ randomBlock[0].type }} -->
          <img
            class="random-cat-img"
            :src="`src/assets/cat/${randomBlock[0].type}`"
            alt=""
          />
        </div>
        <!-- 隐藏 -->
    ...
    <a-row v-if="slotAreaVal.length > 0" align="center" class="slot-board">
      <div v-for="(slotBlock, index) in slotAreaVal" :key="index" class="block">
        ...
        <img
          v-if="slotBlock"
          class="slot-board-img"
          :src="`src/assets/cat/${slotBlock?.type}`"
          alt=""
        />
      </div>
    </a-row>
